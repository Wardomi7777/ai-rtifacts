...
import { useDiagramLLM } from '../../../hooks/useDiagramLLM';
import { VoiceInput } from '../../ui/VoiceInput';

interface DiagramLLMEditorProps {
  source: string;
  onUpdate: (newSource: string) => void;
}

export const DiagramLLMEditor: React.FC<DiagramLLMEditorProps> = ({ source, onUpdate }) => {
  const [question, setQuestion] = useState('');
  const [voiceError, setVoiceError] = useState<string | null>(null);
  const { processDiagram, loading, error } = useDiagramLLM();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await processDiagram(question, source);
      if (result) {
        onUpdate(result.source);
      }
    } catch (err) {
      console.error('Failed to process diagram:', err);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="bg-white/60 backdrop-blur-sm rounded-xl p-6 shadow-sm">
          <label htmlFor="llm-question" className="block text-sm font-medium text-gray-700 mb-2">
            How would you like to modify this diagram?
          </label>
          <div className="flex items-center gap-4">
            <div className="flex-1 min-w-0 flex items-center gap-2 rounded-lg border-0 bg-white shadow-inner ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-purple-600">
              <input
                ...
                placeholder="e.g., Add a new node for error handling, Change the flow direction..."
